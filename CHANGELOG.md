...
- Skill lookup now recursively resolves nested `SKILL.md` files by the YAML
  frontmatter `name` field, including `.md`-suffixed queries and WSL/Windows
  path spellings. Skill saves also synthesize required `name`/`description`
  frontmatter when content is missing it.
